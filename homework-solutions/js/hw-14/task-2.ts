...
function customMap<T, U>(arr: T[], callback: (item: T, index: number) => U): U[] {
  const result: U[] = []; // Новый массив для результатов

  for (let i = 0; i < arr.length; i++) {
    result.push(callback(arr[i], i)); // Вызываем колбэк, кладем в массив
  }
